...
const { getExportConfigById, applyFilters } = require('../customExport.service');
...
/**
 * Turns the connection's export config into the flat list of sellable inventory items.
 * Reuses {@link applyFilters} (published-only parents + published variants + the config's
 * filters), then emits one item per variant — or the parent itself for no-variant products
 * (design §3.4). Items without a SKU are dropped (nothing to match on).
 *
 * @returns {Promise<Array<{ parentCode:string, variantCode:string|null, sku:string,
 *   barcode:string|null, quantity:number }>>}
 */
async function buildInventoryItems(config) {
    const db = getDb();
...
async function executeRun(connection, job, token) {
    ...
    try {
        ...
        const items = await buildInventoryItems(exportConfig);
        ...
        // Push in throttle-aware batches, a few concurrently (queue §8.5).
